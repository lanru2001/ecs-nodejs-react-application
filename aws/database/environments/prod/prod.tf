...
terraform {
 backend "s3" {
    bucket         = "tf-state-file03"  
    key            = "project/prod/rds"
    region         = "us-east-2"
...
module "dev_db"  {
  ...
  db_subnet_group_name            = "prod_db_subnet_group"
  parameter_group_name            = "prod-db-pg"
  maintenance_window              = "Mon:00:00-Mon:03:00"
  secretmanager_name              = "prod/postgres/prod-rds-seed"
